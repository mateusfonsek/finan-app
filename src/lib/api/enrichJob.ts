 ...
 */
export async function startTaxIdEnrichment(
  accountId: number | null,
  onEvent: (event: EnrichEvent) => void,
): Promise<void> {
  const channel = new Channel<EnrichEvent>();
  channel.onmessage = onEvent;
  unwrap(await commands.startTaxIdEnrichment(accountId, channel));
}

export async function cancelTaxIdEnrichment(): Promise<void> {
  await commands.cancelTaxIdEnrichment();
}
